...
def download_image_gdf(start_date: str,
    ...
    for name in image_names:
        ...
        try:
            ...
            print(f"ERROR: Image {name} could not be downloaded")

        # Image processing
        try:
            process_l1c_to_l2a(name)
        except Exception:
            print(f"ERROR: Image {name} could not be processed to L2A")
    ...
    for date in date_identifiers:
        ...
        try:
            ...
            pir = np.array(big_raster_pir.zonal_stats(gdf_municipality, stats=["mean"])["mean"])
            ndvi_val = (pir - red) / (pir + red)
            results["NDVI"].append(ndvi_val)
            #Do the other raster and all the other band and indexes

            dates.append(pd.to_datetime(date, format="%Y\%m\%d"))

        except Exception:
            print(f"ERROR: Image at date {date} could not be analysed")
